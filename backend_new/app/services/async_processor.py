...
class AsyncDataProcessor:
    ...
    async def process_cleaning_job(self, file_id: str, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            def progress_callback(job_id: str, progress: float, message: str, metrics: Dict = None):
                # Ajustar progreso para el rango 10-90%
                adjusted_progress = 10 + (progress * 0.8)
                # Mantener estado en 'en_progreso' durante todo el procesamiento
            ...
            # 8. Completar job incluyendo metadatos útiles para el frontend
            # Mapear campos del cleaning_report a los nombres que espera el frontend
            frontend_metrics = {
            ...
            # Mapear campos del cleaning_report
            if cleaning_report:
    ...
    async def _get_file_info(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Usar archivo local (no Storage por ahora)
            return {
                    "id": file_data["id"],
    ...
    async def _download_file(self, storage_path: str, filename: str) -> Optional[str]:
        """Usar archivo local directamente (sin Storage)"""
        try:
            # Por ahora, no usamos Storage, así que retornamos None
            # El archivo ya está disponible localmente
            return None
    ...
    async def _save_cleaned_file(self, cleaned_data: pd.DataFrame, original_filename: str, file_id: str) -> Dict[str, str]:
        """Guardar archivo limpio en Storage"""
        try:
            ...
            temp_path = Path.cwd() / "temp" / cleaned_filename
            temp_path.parent.mkdir(parents=True, exist_ok=True)
            cleaned_data.to_csv(temp_path, index=False)

...
export const dataApi = {
  ...
  async cleanDataFromDatabase(fileId: string): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/data/clean-data/${fileId}`, {
      method: 'POST',
    });
    if (!response.ok) {
      throw new Error('Failed to clean data from database');
    }
    return response.json();
  },
  ...
  async getCleanedFilePreview(filename: string, limit: number = 10): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch cleaned file preview');
    }
    return response.json();
  },
  ...
  async startCleanDataJob(fileId: string): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/data/jobs/clean-data/${fileId}`, {
      method: 'POST',
    });
    if (!response.ok) {
      throw new Error('Failed to start clean data job');
    }
    return response.json();
  },
  ...
  async getJobsByFile(fileId: string): Promise<PipelineJob[]> {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch jobs by file');
    }
    return response.json();
  },
